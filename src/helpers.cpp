...
bool waitForColor(double hueMin1, double hueMax1,
                  double hueMin2 = -1, double hueMax2 = -1,
    ...
    while (pros::millis() - startTime < timeout) {
        ...
        if (proximity > proximityThreshold) {
            ...
            // Single hue range
            if (hueMin2 < 0) {
                if (hue >= hueMin1 && hue <= hueMax1)
                    colorMatch = true;
            }
            // Two hue ranges (for red wraparound)
            else {
                if ((hue >= hueMin1 && hue <= hueMax1) ||
                    (hue >= hueMin2 && hue <= hueMax2))
                    colorMatch = true;
            }
        }
